...
void Spotify::begin(){ 
  ...
  if(!is_auth()){
    if(_port == 80){
      snprintf(_redirect_uri,sizeof(_redirect_uri), "http://carbruh.local/", WiFi.localIP().toString().c_str());
      Serial.printf("Go to this url in your Browser to login to spotify or enter your credentials: %s\n", _redirect_uri);
    }
    else{
      snprintf(_redirect_uri,sizeof(_redirect_uri), "http://carbruh.local:%d/", WiFi.localIP().toString().c_str(), _port);
      Serial.printf("Go to this url in your Browser to login to spotify or enter your credentials: %s\n", _redirect_uri);
    }
    server_routes();
  }
